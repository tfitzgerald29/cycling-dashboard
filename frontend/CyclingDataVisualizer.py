...
class CyclingDataVisualizer:
    ...
    def create_monthly_distance_plot(self, monthly_data: pd.DataFrame) -> go.Figure:
        ...
        # Update layout
        fig.update_layout(
            legend=dict(orientation="h", yanchor="auto", y=0.99, xanchor="auto", x=0.01)
        )
    ...
    def create_ctl_graph(
        self, merged_df: pd.DataFrame, start_dt: str, end_dt: str
    ) -> go.Figure:
        ...
        fig = px.line(
            filtered_df, x="timestamp", y="CTL", width=1400, height=800, markers=True
        )
    ...
    def create_recent_rides_visualizations(self, recent_rides: pd.DataFrame):
    ...
    def create_daily_distance_plot(self, daily_data: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(
            legend=dict(
                orientation="h", yanchor="auto", y=0.99, xanchor="auto", x=0.01
            ),
            xaxis=dict(
                type="category",
                tickangle=-45,
    ...
    def create_weekly_totals_plot(self, weekly_data: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                ...
                nticks=max(
                    20, min(len(weekly_data), 52)
                ),  # Show up to 52 ticks, but at least 20
            ),
